...
router.post('/CercaUtenza', CercaUtenza);
    ...
    try {
        await withTransaction(db, async() => { 
            ...
            if(req.session.utente){
                results = await db.query('UPDATE utente SET nome = ?,cognome = ?, telefono = ?, regione = ?, provincia = ?, comune = ?, cf = ?, patente = ?,  carta = ?, mese = ?, cvv = ?, email= ?, password = ? WHERE ID_utente = ?',
                [
...
async function autenticazione(req, res, next) {
    ...
    try {
        ...
        await withTransaction(db, async() => {
            // inserimento utente
            string1 = 'accettato'
            results = await db.query('SELECT *,DATE_FORMAT(`utente`.data_nascita,"%d/%m/%Y") AS data_nascita FROM `utente`\
            WHERE email = (?)', [
                    req.body.email
                ])
                .catch(err => {
            ...
            if (results.length == 0) {
              ...
              res.redirect('/accedi')
                    ...
                    req.session.utente = results[0]
                     if (results[0].tipo_utente == "Utente"){
                         ...
                         const today = new Date().getTime();
                             const end = new Date(result[0].al).getTime();
                             var diff = end - today;
                    ...
                    res.redirect('/')}
                            ...
                            result = await db.query('SELECT * from noleggio \
                            WHERE noleggio.stato =  (?) \
   ORDER BY noleggio.al ASC',[
   ...
   if(hours_rem <= 1){    
          ...
          notifica = {
                                counter : 0,
                                string : 'mezzo in ritardo',
                                msg : 'il mezzo : ' + result[0].ID_noleggio + ' è in ritardo',
                            };
...
res.redirect('/modificaNoleggio');
    ...
    noleggio = results[0];
    res.redirect('/ModificaNoleggioAmministratore')
}
});
} catch (err) {
    var message =  'errore di sistema';
...
console.log(req.body.dal,req.body.al,hours1,prezzo1)
            results = await db.query('UPDATE noleggio set noleggio.dal = (?), noleggio.al = (?), noleggio.prezzo = (?), noleggio.dove = (?), noleggio.Ritardo = (?) where ID_noleggio = (?) ', [
                req.body.dal,
                req.body.al,
                prezzo1,
                req.body.Destinazione,
                req.body.MotivoRitardo,
                myIDnoleggio
           ])
           .catch(err => {
               throw err; 
    ...
    if(req.body.mezzo_bici=="si"){
        ...
        if(req.body.mezzo_monopattino=="si"){
            ...
            if (results.length < 1) {
              ...
              res.redirect('/AggiungiParcheggio')
            }
    ...
    try {
      ...
      await withTransaction(db, async() => {
            ...
            if (results.length < 1){
                ...
                notifica = {
                    counter : 0,
                    string : 'Parcheggio non assegnato'
...
try{
    await withTransaction(db, async() => {
        modifica = await db.query('UPDATE mezzi set targa = ?, assicurazione = ?, tariffa = ?, foto = ?, descrizione = ?, guasto = ?\
                                   WHERE ID_mezzo = ?', [
                                ...
                                req.session.mezzo.guasto = req.body.guasto
...
stringG =  'Guasto segnalato: ' + req.body.descrizione_guasto

console.log(stringG)

result = await db.query('UPDATE mezzi set mezzi.guasto = ? WHERE ID_mezzo = ?',[
...
async function CercaUtenza(req, res, next) {
    const db = await makeDb(config);
    let results = {};
                ...
                if (results.length < 1 ){
                    ...
                    res.redirect('/recuperoPassword')
                        ...
                        var mailOptions = {
                          ...
                          text:  'Per recuperare la tua password insersci nella pagina a cui sei stato reindirizzato il seguente token : ' + tkn}
                        ...
                        res.redirect('/ReimpostaPassword')
                        }});
                    }catch (err) {
                        var message =  'errore di sistema';
                        next(createError(message));
...
console.log(results)
                ...
                else if(results[0].tkn != req.body.tkn ){
                    notifica = {
                        string : 'token errato',
                        counter : 0
                    }
